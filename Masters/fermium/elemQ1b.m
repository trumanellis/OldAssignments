function [massM,cF,psi0,mu]=elemQ1b(nodes,QP,wgts2d,refMQ2,GradVBasis,invJ,detJ,...
...
if massUpdate
    for n=1:QP
        massM=massM+wgts2d(n)*Dquad(n)*refMQ2(:,:,n)*detJ(n);
    end
end
...
if forceUpdate
    ...
    for n=1:QP
        ...
        if Qfrac == 1 || hgfrac > 0
            ...
            mu(n)=Dquad(n)*len*(temp2*qquad*abs(DivV)*len+temp3(n)*temp1*qlin*cs(n));
        end
    end
    if Qfrac == 1 
